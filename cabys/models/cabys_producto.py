...
class CabysProducto(models.Model):
    ...
    cabys_enabled = fields.Boolean(default=True)
    cabys_medical = fields.Boolean(default=False)
    ...
    @api.model
    def scheduled_catalog(self):
        values = {'file_url':"https://www.bccr.fi.cr/indicadores-economicos/cabys/Catalogo-de-bienes-servicios.xlsx"}
        record = self.env["cabys.catalog.import.wizard"].create(values)
        _logger.info('\n estoy imprmiendo \n %r \n\n', record)
        # si no es un modelo
        # >> cabys_obj = self.env['cabys.catalog.import.wizard'].browse(record)
        record.download_catalog()
        record.update_catalog()
