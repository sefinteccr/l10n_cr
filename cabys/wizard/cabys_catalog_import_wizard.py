...
class CabysCatalogImportWizard(models.TransientModel):
    ...
    def update_catalog(self):
        ...
        message_cabys = 'El Catálogo Cabys fue actualizado con éxito <br/>'
        if products_new:
            msg += '%s nuevos registros\n' % len(products_new)
            message_cabys += '%s nuevos registros <br/>' % len(products_new)
            message_cabys += 'los elementos nuevos son :  <br/>'
        for vnew in products_new:
            n = ''.join(vnew)
            codigonew = "'"+n+"'"
            query_list_product_new = """SELECT cb.codigo , cb.name FROM cabys_producto cb WHERE cb.codigo = %s""" % codigonew
            self.env.cr.execute(query_list_product_new) 
            line_productnew = self.env.cr.fetchall() 
            listnew.append(line_productnew)
            msg += '%s \n'%line_productnew
            message_cabys += '%s  <br/>'%line_productnew 

        if products_updated:
            msg += '%s registros actualizados\n' % len(products_updated)
            message_cabys += '%s registros actualizados <br/>' % len(products_updated)
            message_cabys += 'los elementos actualizados son :  <br/>'
        for vupdate in products_updated:
            s = ''.join(vupdate)
            codigopu = "'"+s+"'"
            _logger.info('update_catalog %s' % codigopu)
            query_list_product_update = """SELECT cb.codigo , cb.name FROM cabys_producto cb WHERE cb.codigo = %s""" % codigopu
            self.env.cr.execute(query_list_product_update) 
            line_productup = self.env.cr.fetchall() 
            listupdate.append(line_productup)
            msg += '%s \n'%line_productup
            message_cabys += '%s  <br/>'%line_productup 
            
        if products_deleted:
                msg += '%s registros eliminados\n' % len(products_deleted)
                message_cabys += '%s registros eliminados <br/>' % len(products_deleted)
                message_cabys += 'los elementos eliminados son :  <br/>'
        for value in products_deleted:
            codigopd = "'"+value+"'"
            query_list_product_deleted = """SELECT cb.codigo , cb.name FROM cabys_producto cb WHERE cb.codigo = %s""" % codigopd
            self.env.cr.execute(query_list_product_deleted) 
            line_product = self.env.cr.fetchall() 
            listdelete.append(line_product)
            msg += '%s \n'%line_product
            message_cabys += '%s  <br/>'%line_product
            records_ids = self.env['cabys.producto'].search([('codigo', '=', value)])
            records_ids.unlink()
        msg += '\n'
        message_cabys += ' <br/>'
        if categories_new:
            msg += '%s nuevas categorias\n' % len(categories_new)
            message_cabys += '%s nuevas categorias <br/>' % len(categories_new)
        if categories_updated:
            msg += '%s categorias actualizadas\n' % len(categories_updated)
            message_cabys += '%s categorias actualizadas <br/>' % len(categories_updated)
        if categories_deleted:
            msg += '%s categorias eliminadas\n' % len(categories_deleted)
            message_cabys += '%s categorias eliminadas <br/>' % len(categories_deleted)
            

        self.env['mail.message'].sudo().create({
            'subject': 'Actualizacion CABYS',
            'author_id': self.env.user.partner_id.id,
            'model': 'mail.channel',
            'type': 'comment',
            'subtype_id': self.env.ref('mail.mt_comment').id,
            'body': message_cabys,# msg.replace('\n','</br>'), 
            'channel_ids': [(4, self.env.ref('cabys.channel_cabys_all_employees').id)]
        })
