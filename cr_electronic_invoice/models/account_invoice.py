    ...
    def _get_default_activity_id(self):
        for line in self:
            line.economic_activity_id = line.product_id and line.product_id.categ_id and \
                                        line.product_id.categ_id.economic_activity_id and \
                                        line.product_id.categ_id.economic_activity_id.id
...
class AccountInvoiceElectronic(models.Model):
    ...
    # 4.4
    condicionVentaOtros = fields.Char(string="Detalle condicion de la Venta OTRO")
    ...
    def action_invoice_sent(self):
        ...
        if self.env.user.company_id.frm_ws_ambiente == 'disabled':
            email_template.with_context(type='binary', default_type='binary').send_mail(self.id, raise_exception=False,
            ...
            for invoice in self:
                ...
                message = _(
                    "This %s has been created from: <a href=# data-oe-model=account.invoice data-oe-id=%d>%s</a>") % (
                          doc_type[invoice.type], invoice.id, invoice.number)
    ...
    def _check_hacienda_for_invoices(self, max_invoices=10):
        ...
        for i in invoices:
            try:
                current_invoice += 1
                _logger.info('E-INV CR - Consulta Hacienda - Invoice %s / %s  -  number:%s', current_invoice,
                ...
                if estado_m_h == 'aceptado':
                    ...
                    _logger.error('E-INV CR - Consulta Hacienda - Invoice not found: %s  -  Estado Hacienda: %s',
                                  i.number_electronic, estado_m_h)
                ...
                if not inv.xml_comprobante or (inv.tipo_documento == 'FEC' and inv.state_tributacion == 'rechazado'):
                    ...
                    if (inv.invoice_id or inv.not_loaded_invoice) and inv.reference_code_id and inv.reference_document_id:
                        if inv.invoice_id:
                            if inv.invoice_id.number_electronic:
                                ...
                                fecha_emision_referencia = inv.invoice_id.date_issuance or inv.invoice_id.date_invoice.strftime("%Y-%m-%d") + "T12:00:00-06:00"
                            else:
                    ...
                    for inv_line in inv.invoice_line_ids:
                        ...
                        if inv_line.product_id and inv_line.product_id.id == self.env.ref(
                                'cr_electronic_invoice.product_iva_devuelto').id:
                            ...
                            if inv_line.discount_code_id.code:
                                ...
                                if inv_line.discount_code_id.code == '99':
                                        ...
                                        line["naturalezaDescuento"] = inv_line.discount_code_id.name
                    ...
                    if _no_cabys_medical and inv.tipo_documento != 'NC':
                        inv.state_tributacion = 'error'
                        inv.message_post(subject=_('Error'), body=_no_cabys_medical)
                        continue
    ...
    def action_invoice_open(self):
        ...
        for inv in self:
            ...
            if inv.type in ('out_invoice', 'out_refund') and inv.number_electronic:  # Keep original Number Electronic
                ...
                if inv.tipo_documento == 'FE' and (
                        not inv.partner_id.vat or inv.partner_id.identification_id.code == '05'):
    ...
    def _update_hacienda_error_to_process(self, date_from='2023-09-01', date_to='2023-09-06'):
        ...
        invoices.write({'state_tributacion':False,'error_count':0, 'xml_comprobante':False,'xml_respuesta_tributacion':False})
        # query_update = """
        #     UPDATE account_invoice SET state_tributacion = Null, error_count = 0, xml_comprobante=Null
